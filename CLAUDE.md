...
- `OrdersRepository` (`repo/orders_repository.go`) is the pre-existing fat interface, still the default for handlers on `*OrdersAPI`. Adding a `*OrdersDB` method there is still correct while its consumers take it whole — but when the calling package is one of the narrow ones above, add the method to that interface instead
...
`OrdersRepository` (103 methods) is the interface most consumers still take whole. It is being broken up from the consumer side, one package at a time, and it shrinks only when the last consumer of a block of methods has its own interface. Don't refactor it as part of unrelated work.
